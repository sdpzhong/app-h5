...
<script setup lang="ts">
  ...
  function handleExchangeDevice() {
    if (activeItemSn.value) {
      ...
      //   selectPackageId.value = lowPowerPackageList.value[0].id;
      // } else {
      //   selectPackageId.value =
      //     eventRecordPackageList.value[0].id ?? allTimeRecordPackageList.value[0].id;
      // }

      //直接刷新套餐列表
      cloudStorageStore.requestPackageInfo(activeItemSn.value);

      cloudStorageStore.requestDeviceCloudStorageStatusInfo(activeItemSn.value);
    }
    // 关闭弹窗
