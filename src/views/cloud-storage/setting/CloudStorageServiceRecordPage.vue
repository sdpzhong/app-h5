          ...
          <div class="traffic-package-list">
            ...
            >
              <div class="left-section"
                ><div class="package-name">{{ record.packageName || '_' }}</div>
...
<script setup lang="ts">
  import { getDeviceAllCloudStroageRecord } from '@/api/modules/iot-device';
  ...
  const serviceRecords = ref<CloudStorageServiceRecord[]>([]);

  const sn = route.query['sn'] as string | undefined;

  const pageParams: IGetDeviceCloudStorageServiceRecordRequest = {
    current: 1,
    size: 10,
    sorts: '',
    sn,
  ...
  async function requestCsServiceRecord() {
    setLoading(true);
    try {
      const { data, current, pages, size, total } = await getDeviceAllCloudStroageRecord(
        pageParams,
      );
      ...
      console.log('requestCsServiceRecord fail: ', error);
