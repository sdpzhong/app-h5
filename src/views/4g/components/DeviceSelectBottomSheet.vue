...
<script setup lang="ts">
  ...
  const cloudStorageStore = useTrafficPackageStore();
  const { deviceList, selectDevice, selectDeviceSN } = storeToRefs(cloudStorageStore);
  ...
  function handleExchangeDevice() {
    if (activeItemSn.value) {
      ...
      cloudStorageStore.requestDeviceTrafficPackageServiceStatusInfo(activeItemSn.value);
    }
    // 关闭弹窗
