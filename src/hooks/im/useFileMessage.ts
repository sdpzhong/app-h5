...
import { base64toFile } from '@/utils/bas64ToFile';
import { IMSDK } from '@/utils/open-im';
...
export function useFileMessage() {
  ...
  const createImageOrVideoMessage = async (file: FileWithPath) => {
  ...
  const getVideoSnshotFile = async (file: File): Promise<File> => {
